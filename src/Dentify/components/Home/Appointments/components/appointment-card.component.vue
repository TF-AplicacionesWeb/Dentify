  ...
  <div class="bg-[#d1f2eb] min-h-screen mt-28 p-8">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      >
        ...
        <p class="text-[#082f49]"><strong>Appointment Date:</strong> {{ formatDate(appointment.appointment_date) }}</p>
      ...
      <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
        ...
        <form @submit.prevent="addAppointment">
          <div class="mb-4">
            ...
            <input v-model="newAppointment.dentist_dni" type="text" class="w-full border border-gray-300 rounded px-3 py-2" required />
          </div>
          <div class="mb-4">
            <label class="block text-[#082f49] mb-1">Patient DNI</label>
            <input v-model="newAppointment.patient_dni" type="text" class="w-full border border-gray-300 rounded px-3 py-2" required />
          </div>
          <div class="mb-4">
            <label class="block text-[#082f49] mb-1">Appointment Date</label>
            <input v-model="newAppointment.appointment_date" type="datetime-local" class="w-full border border-gray-300 rounded px-3 py-2" required />
          </div>
          <div class="mb-4">
            <label class="block text-[#082f49] mb-1">Reason</label>
            <input v-model="newAppointment.reason" type="text" class="w-full border border-gray-300 rounded px-3 py-2" required />
          </div>
          <div class="mb-4">
            <label class="block text-[#082f49] mb-1">Duration (minutes)</label>
            <input v-model="newAppointment.duration_minutes" type="number" class="w-full border border-gray-300 rounded px-3 py-2" required />
          </div>
